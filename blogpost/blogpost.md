...
How to integrate the parallelity and sequentiality of $\mathbf{C}_\text{det}$ to $\mathbf{C}$? 
...
<!-- Should I change the example to account for this? -Utku -->
...
Before we talk about what this means in detail, let's examine two properties that do *not* hold:
...
The above equation will serve as definition of "deterministic morphisms" in general Markov categories, as ...
 2. ... there are Markov categories that *do not* arise as Kleisli categories: e.g., $\mathbf{FinStoch}$ of finite sets and Markov kernels does have $$\mathbf{FinStoch}_{det} = \mathbf{FinSet} \subset \mathbf{FinStoch}$$ as wide subcategory of deterministic morphisms, the inclusion cannot have a right adjoint (because the set $PX$ of distributions over $X$ is infinite).
...
<!-- Do we want to just mention these briefly instead of delving deep? How
about linking to one of Paolo's papers for more details and just outsourcing it? :) -->
